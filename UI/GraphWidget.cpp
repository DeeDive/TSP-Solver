...
void GraphWidget::setNodes(const std::vector<Node>& nodes)
{
    ...
    for(auto& n : nodes)
    {
        ...
        node->setID(n.idx);
        scene_->addItem(node);
        nodes_.insert(std::make_pair(n.idx, node));
    }
}
...
void GraphWidget::setEdges(const std::vector<Node>& circuit)
{
    clearEdges();
    for(auto i = 0u; i < circuit.size() - 1; ++i) {
        auto n1 = nodes_[circuit[i].idx];
        auto n2 = nodes_[circuit[i + 1].idx];
        GUIEdge* edge = new GUIEdge(n1, n2);
        scene_->addItem(edge);
        gaEdges_.push_back(edge);
    }
//    GUIEdge* edge = new GUIEdge(nodes_[circuit[circuit.size()-1].idx],nodes_[circuit[0u].idx]);
//    scene_->addItem(edge);
//    gaEdges_.push_back(edge);
}
